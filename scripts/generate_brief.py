...
def write_csvs(data: dict[str, pd.DataFrame], data_dir: Path) -> Path:
    ...
    for metric in METRICS:
        ...
        rows.append({
            ...
            "unit": metric.unit,
            "daily_change_pct": stats.daily_change_pct(df),
            "weekly_change_pct": stats.change_over_pct(df, 5),
            "monthly_change_pct": stats.change_over_pct(df, 21),
...
def _row(metric, df) -> str:
    ...
    w1 = stats.change_over_pct(df, 5)
    p = stats.percentile_rank(df)
    return (
        f"| {metric.short_name} | {last:,.2f} | {metric.unit} | "
        f"{_fmt_pct(d1)} | {_fmt_pct(w1)} | {_fmt_int(p)} | {sig.headline} |"
    )
...
def build_markdown(
    ...
    if narrative.source == "claude":
        L(f"_Generated by **{narrative.model}** via Anthropic API. Prompts/responses logged "
          f"to `ai/logs/`._")
    else:
    ...
    L("## 2 · Monitor metrics")
    L("")
    L("| Metric | Latest | Unit | 1d Δ | 1w Δ | 5y pctile | Headline |")
    L("|---|---:|---|---:|---:|---:|---|")
    for metric in METRICS:
        L(_row(metric, data.get(metric.key)))
    L("")
    L(f"_Full 5-year history per metric in `data/<metric>.csv`. "
    ...
    L("- Coal: ICE Newcastle (proxy for API2 — best free daily source; ~0.85 historical correlation)")
